...
function TopLosersCard({ Icon, title, coins }) {
  var data = coins
  var res = alasql(
    "SELECT TOP 4 * FROM ? ORDER BY price_change_percentage_24h ASC",
    [data]
  )
  return (
    <div>
      <Link to="/allcoins/">
        <div className="card rounded-lg shadow-xl">
          <div className="flex flex-row justify-center">
            <Icon className="text-red-500 m-0.5 text-xl" />
            <div className="text-gray-600 font-bold text-sm ml-2">{title}</div>
          </div>
            ...
            {res.map(coin => (
              <div key={coin.market_cap_rank}>
                <div className="card-headline grid grid-cols-4 font-semibold pl-1 mb-1 pb-1">
                  ...
                  <div className="font-bold text-red-600 text-left">
                    ₹ {coin.current_price}
                  </div>
                  <div className="text-red-600 flex flex-row text-left">
